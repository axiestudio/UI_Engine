...
Two-door airlock auth cycle — inner door opens only after outer seals
...
From the repo root: `npm run build:airlock-cycle` (or `npm run build:presets`). Engine consumes it via the `file:` dependency.
