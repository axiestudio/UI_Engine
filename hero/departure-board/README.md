...
Split-flap departure-board headlines that scramble and settle — program/live-data masthead (site section + webapp status header)
...
Category `hero` · motion via `motion/react` · brand tokens in `src/index.css` (`:root` + `.dark`).

## Install & build

From the repo root: `npm run build:departure-board` (or `npm run build:presets`). Engine consumes it via the `file:` dependency.
