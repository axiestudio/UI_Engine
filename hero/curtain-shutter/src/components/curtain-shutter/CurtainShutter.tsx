...
export type CurtainShutterProps = {
  mode?: "overlay" | "stage"
...
export function CurtainShutter({
  mode = "overlay",
  logo,
  kicker = "UI ENGINE",
  title = "WELCOME",
  description = "A token-first collection of pre-built website sections. Scroll to open the blinds.",
  enter = { label: "Enter" },
  ...
  return (
    ...
    >
      <div
        ...
        aria-label={mode === "overlay" ? "Intro shutter" : undefined}
      >
        <ShutterScene
          progress={progress}
          reduce={reduce}
...
function ShutterScene({
  ...
  title: React.ReactNode
  description: React.ReactNode
  enter: NonNullable<CurtainShutterProps["enter"]>
  hint: string
  onEnter: () => void
  onEnterKey: boolean
}) {
  const slatCount = Math.max(slats, 2)
  // each slat starts to open slightly after the previous: stagger across progress
  const textOpacity = useTransform(progress, [0, 0.6], [1, 0])
  const textScale = useTransform(progress, [0, 1], [0.94, 1.12])
    ...
    <>
      <div className="absolute inset-0 flex flex-col">
        {Array.from({ length: slatCount }).map((_, i) => {
          const start = (i / slatCount) * 0.4
          const end = start + 0.6
          const y = useTransform(progress, [start, end], ["0%", "-110%"])
          const opacity = useTransform(progress, [start, Math.min(end, start + 0.4)], [1, 0.9])
          return (
            <motion.div
              key={i}
              style={{ y, opacity }}
              className="relative flex-1 overflow-hidden bg-[hsl(var(--curtain))] will-change-transform"
            >
              <div className="absolute inset-0" style={{ background: `linear-gradient(180deg, hsl(var(--curtain-lit)) 0%, hsl(var(--curtain)) 60%)` }} />
