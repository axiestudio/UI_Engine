      ...
      <InView once variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
        <div className={cn("flex flex-col items-center justify-between gap-4 rounded-2xl border px-6 py-5 sm:flex-row", ink ? "border-background/20 bg-background/5" : "border-border bg-card")}>
          <div className="flex items-center gap-3">
            <span className={cn("flex h-9 w-9 items-center justify-center rounded-xl", ink ? "bg-background/10" : "bg-accent")}><Zap className="h-4 w-4 text-amber-500" /></span>
            <div>
              <p className="font-display text-base font-bold">{text}</p>
              {deadline && <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{deadline}</p>}
            </div>
          </div>
          <button type="button" onClick={copy} className={cn("rounded-full border px-4 py-2 font-mono text-[11px] font-bold uppercase tracking-widest transition-colors", ink ? "border-background/30" : "border-foreground/30")} aria-label="Copy promo code">
