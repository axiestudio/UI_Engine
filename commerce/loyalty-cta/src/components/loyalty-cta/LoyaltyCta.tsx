...
export function LoyaltyCta({ eyebrow = "REWARDS", title = "Points with every purchase", body = "Points on every order — redeem for discounts, redeemable for the next one.", points = "1,240 pts", cta = "Join the club", tone = "paper", className }: LoyaltyCtaProps) {
  const ink = tone === "ink"
  return (
    <SectionShell tone={tone} width={920} grain={!ink} rule="bottom" className={className}>
      <InView once variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
        <div className={cn("relative overflow-hidden rounded-2xl border p-10 text-center", ink ? "border-background/25 bg-background/5" : "border-border bg-card")}>
          ...
          {points && <p className="mt-6 inline-block rounded-full bg-success/10 px-4 py-1.5 font-display text-lg font-semibold text-success">{points}</p>}
          <div className="mt-6"><Button size="lg" className="h-11 rounded-full px-7 font-mono text-[11px] font-bold uppercase tracking-[0.12em]">{cta}</Button></div>
        </div>
      </InView>
    </SectionShell>
  )
