...
export function CommercePricingToggle({ eyebrow = "PRICING", title = "Pick a plan, switch anytime.", tiers, className }: CommercePricingToggleProps) {
      ...
      <InView once variants={{ hidden: { opacity: 0, y: 12 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <div className="mt-6 inline-flex rounded-full border p-1">
          {["monthly", "annual"].map((m) => {
            ...
            return <button key={m} type="button" onClick={() => setAnnual(isAnnual)} className={cn("rounded-full px-5 py-2 font-mono text-[11px] font-bold uppercase tracking-widest transition-colors", annual === isAnnual ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground")}>{m}</button>
          })}
        </div>
      </InView>
        ...
        {tiers.map((t, i) => (
          <InView key={t.id} once variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: i * 0.06 }}>
            <div className={cn("flex h-full flex-col rounded-2xl border p-6", t.highlight ? "border-2 border-foreground bg-card" : "border-border bg-card")}>
              {t.highlight && <span className="mb-3 inline-block w-fit rounded-full bg-emerald-500/10 px-2.5 py-1 font-mono text-[9px] font-bold uppercase tracking-widest text-emerald-600">Popular</span>}
              ...
              {annual && <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-emerald-600">billed annually</p>}
              <ul className="mt-5 flex-1 space-y-2">
                {t.perks?.map((p) => <li key={p} className="flex items-center gap-2 text-sm font-medium"><Check className="h-3.5 w-3.5 text-emerald-500" /> {p}</li>)}
              </ul>
              <Button variant={t.highlight ? "default" : "outline"} className="mt-6 h-10 rounded-full font-mono text-[10px] font-bold uppercase tracking-widest">Choose</Button>
            </div>
          </InView>
        ))}
      </div>
