...
export function Tomato({ label = "Heirloom tomato", className }: TomatoProps) {
  return (
    <div className={cn("relative h-4 w-full", className)} title={label}>
      <div className="absolute inset-x-1.5 inset-y-0 rounded-full bg-[#d84a35]" />
      {["12%", "30%", "48%", "66%", "84%"].map((left) => (
        <span key={left} className="absolute top-1/2 h-[5px] w-[5px] -translate-y-1/2 rounded-full bg-[#f0b46a]" style={{ left }} />
      ))}
    </div>
  )
}
