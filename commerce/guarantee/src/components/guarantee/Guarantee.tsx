...
export function Guarantee({ eyebrow = "NOT RISKY", title = "Love it, or it's on us.", body = "If a section doesn't fit your build, we'll refund you — no forms, no fuss.", terms = ["30 days", "No questions", "Full refund"], tone = "paper", className }: GuaranteeProps) {
  const ink = tone === "ink"
  return (
    <SectionShell tone={tone} width={920} grain={!ink} rule="bottom" className={className}>
      <InView once variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
        <div className="rounded-2xl border p-10 text-center">
          ...
          <h2 className="mt-3 font-display text-3xl font-black tracking-[-0.03em] sm:text-5xl">{title}</h2>
          ...
          <ul className="mt-8 flex flex-wrap justify-center gap-3">
            {terms.map((t) => (
              <li key={t} className="rounded-full border px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest">{t}</li>
            ))}
          </ul>
        </div>
      </InView>
