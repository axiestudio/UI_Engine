        ...
        {featured && (
          <div className={cn("flex flex-col rounded-2xl border-2 p-8", ink ? "border-background/30 bg-background/5" : "border-foreground bg-card")}>
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-emerald-500/10 px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-widest text-emerald-600">Most popular</span>
            </div>
            <p className="mt-4 font-display text-5xl font-black">{featured.price}<span className="text-lg text-muted-foreground">{featured.period}</span></p>
            ...
            <Button size="lg" className="mt-6 h-11 rounded-full font-mono text-[11px] font-bold uppercase tracking-widest">{cta}</Button>
          </div>
        )}
          ...
          {rest.map((o) => (
            <div key={o.id} className={cn("flex flex-1 items-center justify-between gap-4 rounded-2xl border p-6", ink ? "border-background/15 bg-background/5" : "border-border bg-card")}>
              ...
              <div className="text-right">
                <p className="font-display text-2xl font-black">{o.price}</p>
                <Button variant="outline" size="sm" className="mt-2 rounded-full font-mono text-[10px] font-bold uppercase tracking-widest">{cta}</Button>
              </div>
            </div>
          ))}
        </div>
