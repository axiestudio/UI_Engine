...
function PlanCard({ plan, yearly, currency, periodLabel }: { plan: PricingPlan; yearly: boolean; currency: string; periodLabel: string }) {
  ...
  const cta = plan.cta.href ? (
    <Button asChild className="h-11 w-full rounded-full font-display text-sm font-extrabold tracking-tight" variant={plan.popular ? "default" : "outline"}>
      <a href={plan.cta.href} onClick={plan.cta.onClick}>{plan.cta.label}</a>
    </Button>
  ) : (
    <Button onClick={plan.cta.onClick} variant={plan.popular ? "default" : "outline"} className="h-11 w-full rounded-full font-display text-sm font-extrabold tracking-tight">
      {plan.cta.label}
    </Button>
  )
  return (
    <div
      id={plan.id}
      className={cn(
        "relative flex h-full scroll-mt-24 flex-col rounded-[24px] border p-6 shadow-sm transition-shadow hover:shadow-md sm:p-7",
        plan.popular ? "border-foreground bg-card ring-1 ring-foreground" : "bg-card"
      )}
    >
      {plan.popular && (
        <span className="absolute -top-3 left-6 rounded-full bg-foreground px-2.5 py-1 font-mono text-[10px] font-black uppercase tracking-widest text-background shadow">
          Most popular
        </span>
      )}
      ...
      <p className="mt-6 flex items-baseline gap-2">
        <span className="font-display text-[38px] font-black leading-none tracking-[-0.04em] tabular-nums">{formatPrice(price, currency)}</span>
        <span className="font-mono text-[11px] font-bold uppercase tracking-widest text-muted-foreground">{periodLabel}</span>
      </p>
      ...
      <InView variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} viewOptions={{ once: true, margin: "-80px" }}>
        <div className="mx-auto w-full max-w-[1120px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <header className="mx-auto mb-10 max-w-2xl text-center">
            ...
            <h2 id="pricing-title" className="mt-2 font-display text-3xl font-extrabold tracking-tight sm:text-4xl">{title}</h2>
            ...
            {showToggle && (
              <div className="mt-7 inline-flex items-center gap-3 rounded-full border bg-card px-4 py-2 shadow-xs" role="group" aria-label="Billing period">
                <span className={cn("text-xs font-bold uppercase tracking-widest", !yearly ? "text-foreground" : "text-muted-foreground")}>Monthly</span>
                <Switch checked={yearly} onCheckedChange={(v) => setYearly(v)} aria-label={yearly ? "Switch to monthly billing" : "Switch to yearly billing"} />
                <span className={cn("text-xs font-bold uppercase tracking-widest", yearly ? "text-foreground" : "text-muted-foreground")}>Yearly</span>
                {saveLabel && yearly && <span className="rounded-full bg-foreground px-2 py-0.5 text-[10px] font-black text-background">{saveLabel}</span>}
              </div>
            )}
          </header>
