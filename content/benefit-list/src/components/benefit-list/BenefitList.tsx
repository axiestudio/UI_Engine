...
export function BenefitList({ eyebrow = "BENEFITS", title = "What you get.", items = [
  ...
  <div className={cn("relative mx-auto w-full px-4 sm:px-6 lg:px-8", "py-20 sm:py-24")} style={{ maxWidth: (1120), ["--shell-w" as string]: `${(1120)}px` }}>
      ...
      <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
        <InView once viewOptions={{ margin: "-60px" }} variants={{ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
    ...
    <h2 className={cn("font-display text-[34px] font-black leading-[0.98] tracking-[-0.035em] sm:text-[44px] lg:text-[52px]", tone === 'ink' ? "text-background" : "text-foreground")}>{title}</h2>
        ...
        <InView once viewOptions={{ margin: "-60px" }} variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1], delay: 0.08 }}>
          <ul className="grid grid-cols-1 gap-x-8 gap-y-4 sm:grid-cols-2">
            {items.map((it) => (
              <li key={it} className={cn("flex items-start gap-3 text-sm font-medium leading-relaxed", ink ? "text-background/80" : "text-foreground")}>
                <Check aria-hidden="true" className="h-4 w-4 shrink-0 text-emerald-500" strokeWidth={2} />
                {it}
              </li>
            ))}
          </ul>
