  ...
  return (
    <SectionShell tone={tone} width={1120} grain={!ink} rule="bottom" className={className}>
      ...
      <InView once variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1], delay: 0.1 }}>
        <div className={cn("mt-10 grid gap-px overflow-hidden rounded-2xl border", cols)}>
          {logos.map((l) => (
            <div key={l} className={cn("flex items-center justify-center px-4 py-8", ink ? "border-background/10 bg-background/5" : "border-border bg-card")}>
              <span className="font-display text-sm font-black uppercase tracking-[0.18em] opacity-60">{l}</span>
            </div>
          ))}
        </div>
      </InView>
