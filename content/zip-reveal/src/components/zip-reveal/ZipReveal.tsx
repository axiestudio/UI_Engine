...
export function ZipReveal({ eyebrow = "UNSEALED ON ARRIVAL", label = "drop-01.zip", title = "The contents were worth the wait.", height = "200vh", open, onOpen, children, className }: ZipRevealProps) {
  ...
  const Parcel = (
    <div ref={trackRef} className="relative w-full overflow-hidden rounded-lg border shadow-[0_30px_70px_-30px_hsl(var(--foreground)/0.5)]">
          ...
          <>
            ...
            <ul className="mt-5 grid gap-2 font-mono text-[11px] font-bold uppercase tracking-[0.12em] sm:grid-cols-2">
              {["01 / lookbook-cover.webp", "02 / studio-reel.mp4", "03 / colour-bar-cuts", "04 / press-kit.pdf"].map((f) => (
                <li key={f} className="flex items-center gap-2 rounded-sm bg-foreground/10 px-3 py-2"><span className="size-1.5 rounded-full bg-current opacity-60" /> {f}</li>
              ))}
            </ul>
          </>
        )}
      ...
      <motion.div aria-hidden style={{ clipPath: flap }} className="absolute inset-0 z-[1]">
        ...
        <span className="absolute left-8 top-8 -rotate-3 border border-dashed border-[hsl(var(--parcel-ink))/0.5] bg-background/70 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-[hsl(var(--parcel-ink))]">{label}</span>
        <span className="absolute inset-y-0 left-1/2 w-10 -translate-x-1/2 -rotate-2 bg-background/25 mix-blend-multiply" />
  ...
  if (controlled) return <div className={cn("relative isolate w-full overflow-hidden py-2", className)}>{Parcel}</div>
  return (
    <div ref={wrapRef} className={cn("relative isolate w-full min-h-[400px]", className)} style={{ height }}>
...
function PullKnob({ z, onPointerDown, zNow, controlled }: { z: MotionValue<number>; onPointerDown: (e: React.PointerEvent) => void; zNow: number; controlled: boolean }) {
  ...
  return (
    <motion.button
      ...
      style={{ left }}
      className="absolute top-1/2 z-[2] grid size-11 -translate-x-1/2 -translate-y-1/2 cursor-grab touch-none place-items-center rounded-md border-2 border-[hsl(var(--zip-tape))] bg-gradient-to-b from-[hsl(var(--zip-teeth))] to-[hsl(var(--zip-tape))] shadow-[0_6px_14px_hsl(var(--foreground)/0.4)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[hsl(var(--zip-tape))] active:cursor-grabbing"
    >
