...
export function InteractiveTabsStickyNav({ eyebrow = "STICKY", title = "Tabs that stay within reach.", tabs, onView, className }: InteractiveTabsStickyNavProps) {
  ...
  return (
    <SectionShell width={1120} grain rule="bottom" className={className}>
      ...
      <div className="sticky top-16 z-30 mt-10 -mx-2 overflow-x-auto rounded-2xl border bg-background/85 p-1 backdrop-blur">
      ...
      <div className="mt-8 space-y-8">
        {tabs.map((t) => (
          <div key={t.id} id={`panel-${t.id}`} className={cn("scroll-mt-40 rounded-2xl border p-6 transition-opacity sm:p-8", active === t.id ? "bg-card" : "opacity-60")}>
            <div className="flex items-center gap-4">
              <span className="font-display text-5xl font-black opacity-15">{String(tabs.indexOf(t) + 1).padStart(2, "0")}</span>
