...
Projector film-strip scrub — case studies and tours advance frame by frame with the scroll
...
From the repo root: `npm run build:film-strip-wind` (or `npm run build:presets`). Engine consumes it via the `file:` dependency.
