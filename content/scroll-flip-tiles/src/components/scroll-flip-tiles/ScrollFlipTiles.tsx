          ...
          <InView key={t.id} once variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: i * 0.05 }}>
            <div className="group h-56 [perspective:1200px]">
              <div className="relative h-full w-full transition-transform duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                <div className={cn("absolute inset-0 flex flex-col justify-between rounded-2xl border p-6 [backface-visibility:hidden]", ink ? "border-background/15 bg-card" : "border-border bg-card")}>
                  {t.tag && <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{t.tag}</span>}
                  <h3 className="font-display text-2xl font-black">{t.title}</h3>
                  <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground">HOVER →</span>
                </div>
                <div className={cn("absolute inset-0 flex flex-col justify-center rounded-2xl border p-6 [backface-visibility:hidden] [transform:rotateY(180deg)]", ink ? "inner:border-background/20" : "inner:border-border", "bg-foreground text-background")}>
