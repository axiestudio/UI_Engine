    ...
    <SectionShell tone={tone} width={1120} grain={!ink} rule="bottom" className={className}>
      ...
      <InView once variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
        <dl className="mt-10 grid grid-cols-2 gap-6 sm:grid-cols-4">
          {stats.map((s) => (
            <div key={s.id} className={cn("rounded-2xl border p-6 text-center", ink ? "border-background/15 bg-background/5" : "border-border bg-card")}>
              <dd className="font-display text-4xl font-black tabular-nums sm:text-5xl">
