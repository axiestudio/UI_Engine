...
Museum wall hang — art swings in on wire under a single label plate
...
From the repo root: `npm run build:gallery-rails` (or `npm run build:presets`). Engine consumes it via the `file:` dependency.
