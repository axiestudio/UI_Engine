      ...
      <InView once variants={{ hidden: { opacity: 0, scale: 0.98 }, visible: { opacity: 1, scale: 1 } }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}>
        <div ref={emblaRef} className="relative mt-10 overflow-hidden rounded-xl border bg-foreground">
          ...
          <div className="flex justify-center gap-2 py-3">
            {frames.map((f, i) => <Button type='button' key={f.id} onClick={() => setIdx(i)} aria-label={`Frame ${i + 1}`} className={cn("h-1.5 rounded-full transition-all", i === idx ? "w-6 bg-foreground" : "w-1.5 bg-muted-foreground/40")} size="icon" variant="default"></Button>)}
          </div>
        </div>
      </InView>
