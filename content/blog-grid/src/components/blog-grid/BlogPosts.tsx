        ...
        {featured && (
          <InView once variants={{ hidden: { opacity: 0, y: 18 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
            <a href="#" className="group block">
              <div className="img-hover-wash aspect-[16/10] overflow-hidden rounded-2xl border bg-muted">
              ...
              <div className="mt-4">
                ...
                <h3 className="mt-2 font-display text-2xl font-black leading-tight group-hover:underline">{featured.title}</h3>
        ...
        <div className="grid gap-4">
          {rest.map((p, i) => (
            <InView key={p.id} once variants={{ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: i * 0.05 }}>
              <a href="#" className={cn("group flex gap-4 rounded-2xl border p-3", ink ? "border-background/15 bg-background/5" : "border-border bg-card")}>
