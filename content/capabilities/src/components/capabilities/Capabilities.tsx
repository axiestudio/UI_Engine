      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-3">
        {items.map((c, i) => (
          <InView key={c.id} once variants={{ hidden: { opacity: 0, y: 18 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: i * 0.06 }}>
            <div className={cn("relative h-full overflow-hidden rounded-2xl border p-6", ink ? "border-background/15 bg-background/5" : "border-border bg-card")}>
              <BorderTrail size={52} className={cn("bg-[hsl(var(--site-accent)/0.7)]")} transition={{ ease: "linear", duration: 4 }} />
