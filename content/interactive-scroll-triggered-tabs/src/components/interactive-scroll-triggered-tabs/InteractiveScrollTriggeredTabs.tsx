  ...
  return (
    <SectionShell width={1120} grain rule="bottom" className={className}>
      ...
      <div ref={wrap} className="mt-10 grid gap-8 lg:grid-cols-[240px_1fr]">
        <div className="sticky top-20 h-fit rounded-2xl border bg-card p-2">
          {rows.map((r, i) => (
        ...
        <div className="space-y-6">
          {rows.map((r, i) => (
            <div key={r.id} data-row className={cn("scroll-mt-24 rounded-2xl border p-6 transition-colors", i === active ? "border-foreground bg-card" : "border-border")}>
              <h3 className="font-display text-2xl font-black">{r.title}</h3>
