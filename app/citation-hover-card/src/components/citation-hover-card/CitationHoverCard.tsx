    ...
    <div ref={host} className={cn("relative text-[13px] leading-[1.75] font-sans", className)} onMouseLeave={() => setOpen(null)}>
      ...
      <AnimatePresence>
        {open !== null && pos && sources.find((s) => s.n === open) && (
          <motion.aside initial={{ opacity: 0, y: 6, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, transition: { duration: 0.1 } }} transition={{ type: "spring", stiffness: 380, damping: 26 }} className="absolute z-30 w-72 -translate-x-1/2 rounded-xl border bg-popover p-3.5 shadow-2xl" style={{ left: Math.max(140, Math.min(pos.x, (host.current?.clientWidth ?? 600) - 140)), bottom: pos.bottom }}>
            <p className="flex items-start gap-2 text-[12px] font-bold leading-snug"><Globe aria-hidden className="mt-0.5 size-3.5 shrink-0 text-muted-foreground" />{sources.find((s) => s.n === open)!.title}</p>
            <p className="mt-1.5 line-clamp-3 font-serif text-[11px] italic text-muted-foreground">“{sources.find((s) => s.n === open)!.snippet}”</p>
            <p className="mt-2 flex items-center justify-between font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">{sources.find((s) => s.n === open)!.domain}{[0, 1, 2].map((d) => <span key={d} className="size-1 rounded-full bg-current opacity-30" />)}</p>
            {sources.find((s) => s.n === open)!.href && <a href={sources.find((s) => s.n === open)!.href} className="mt-1 flex items-center gap-1 text-[10px] font-bold text-[hsl(var(--info))] hover:underline">open source <ExternalLink className="size-3" /></a>}
          </motion.aside>
        )}
      </AnimatePresence>
    </div>
