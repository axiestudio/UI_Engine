...
export function StatusHealthStrip({ services, region, onRegion, className }: StatusHealthStripProps) {
  ...
  return (
    <div className={cn("border-b bg-muted/40 font-sans", className)} role="status" aria-label="System health">
      <div className="mx-auto flex h-9 w-full max-w-[1200px] items-center gap-3 px-4 text-[11px]">
        <span className="flex items-center gap-1.5 font-mono font-black uppercase tracking-[0.16em] text-muted-foreground">
        ...
        <div className="ml-auto flex items-center gap-2">
          {onRegion && regions.length > 0 && (
            <label className="flex items-center gap-1 text-muted-foreground">
              ...
              <select value={region ?? "all"} onChange={(e) => onRegion(e.target.value === "all" ? "" : e.target.value)} className="rounded border bg-background px-1.5 py-0.5 font-mono text-[10px] uppercase">
                <option value="all">All regions</option>
                {regions.map((r) => <option key={r} value={r}>{r}</option>)}
              </select>
            </label>
          )}
          <button onClick={() => setOpen((v) => !v)} aria-expanded={open} className="flex items-center gap-1 rounded px-2 py-1 font-mono font-bold uppercase tracking-[0.14em] text-muted-foreground hover:bg-muted">
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.ul initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }} className="overflow-hidden border-t bg-background">
            {shown.map((s) => (
              <li key={s.name} className="flex items-center gap-3 px-4 py-2 text-[12px] odd:bg-muted/30">
                <span className={cn("size-2 shrink-0 rounded-full", DOT[s.state])} aria-hidden />
                <span className="w-44 shrink-0 font-semibold">{s.name}</span>
                {s.region && <span className="hidden w-20 shrink-0 font-mono text-[10px] uppercase text-muted-foreground sm:block">{s.region}</span>}
