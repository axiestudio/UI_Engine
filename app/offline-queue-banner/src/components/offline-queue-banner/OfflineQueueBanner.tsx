    ...
    <AnimatePresence initial={false}>
      {show && (
        <motion.div role="status" aria-live="polite" initial={{ y: -34, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -34, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className={cn("relative w-full overflow-hidden font-sans", className)}>
          <div className={cn("flex items-center gap-2.5 px-4 py-2 text-[12px] font-bold", !online ? "bg-[hsl(var(--warn)/0.15)] text-[hsl(var(--warn))]" : flushing ? "bg-[hsl(var(--info)/0.12)] text-[hsl(var(--info))]" : "bg-[hsl(var(--ok)/0.12)] text-[hsl(var(--ok))]")}>
            {online ? <Wifi aria-hidden className="size-4" /> : <CloudOff aria-hidden className="size-4" />}
            {!online ? <>You’re offline — <motion.span key={queued} initial={{ scale: 1.35, color: "inherit" }} animate={{ scale: 1 }} className="font-mono font-black tabular-nums">{queued}</motion.span> change{queued === 1 ? "" : "s"} will sync when you’re back</> : flushing ? <>Pushing {queued} pending change{queued === 1 ? "" : "s"}…</> : <>{queued} change{queued === 1 ? "" : "s"} queued — retry now?</>}
            {online && !flushing && onRetryNow && <button onClick={onRetryNow} className="ml-auto flex items-center gap-1.5 rounded-full bg-current/10 px-3 py-1 text-[10px] uppercase tracking-[0.12em] hover:bg-current/20"><RefreshCw className="size-3" /> retry</button>}
