...
export function AiPromptComposer({ value, onChange, onSend, busy, models = [], model, onModel, attachments = [], onAttach, contextLimit = 8000, className }: AiPromptComposerProps) {
  ...
  return (
    <div className={cn("rounded-2xl border bg-card shadow-[0_10px_40px_-18px_rgba(0,0,0,0.25)] transition-opacity", busy && "opacity-70", className)}>
      {attachments.length > 0 && (
        <div className="flex gap-2 overflow-x-auto border-b px-3.5 py-2.5">
          {attachments.map((a, i) => (
            <motion.span key={a} initial={{ scale: 0.8, rotate: 0 }} animate={{ rotate: (i - attachments.length / 2) * 1.6, scale: 1 }} className="flex items-center gap-1.5 whitespace-nowrap rounded-lg border bg-[hsl(var(--app-code))] px-2.5 py-1.5 text-[11px] font-bold"><Paperclip className="size-3" aria-hidden /> {a}</motion.span>
          ))}
        </div>
      )}
      <textarea ref={ta} rows={1} value={value} onChange={(e) => onChange(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (!busy && value.trim()) onSend() } }} placeholder="Ask for something specific — context is cheap, clarity is not." aria-label="Prompt" className="max-h-[200px] w-full resize-none bg-transparent px-4 pb-2 pt-3.5 text-[14px] leading-relaxed outline-none" />
      <div className="flex items-center gap-2 px-3 pb-3">
        {onAttach && <button aria-label="Attach file" onClick={onAttach} className="grid size-8 place-items-center rounded-full text-muted-foreground hover:bg-muted"><Paperclip className="size-4" /></button>}
        {models.length > 0 && (
          <button onClick={cycleModel} className="flex h-8 items-center gap-1.5 rounded-full border px-3 text-[11px] font-bold hover:bg-muted">
        ...
        <span className="ml-auto font-mono text-[10px] tabular-nums text-muted-foreground">{used.toLocaleString()} tk</span>
        <span className="relative grid size-9 place-items-center">
          <svg aria-hidden viewBox="0 0 36 36" className="absolute inset-0 -rotate-90"><circle cx="18" cy="18" r="16" fill="none" stroke="hsl(var(--muted))" strokeWidth="3" /><motion.circle cx="18" cy="18" r="16" fill="none" stroke={hot ? "hsl(var(--err))" : "hsl(var(--app-focus))"} strokeWidth="3" strokeLinecap="round" strokeDasharray={`${pct * 100.5} 100.5`} /></svg>
