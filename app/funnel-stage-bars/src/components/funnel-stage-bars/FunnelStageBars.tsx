  ...
  return (
    <figure className={cn("font-sans", className)}>
      <figcaption className="mb-4 flex items-baseline justify-between">
        <p className="font-mono text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">{eyebrow}</p>
        {topLabel && <p className="text-[12px] font-bold">{topLabel}: <span className="text-[hsl(var(--info))]">{(((stages.at(-1)?.value ?? 0) / top) * 100).toFixed(1)}%</span></p>}
      </figcaption>
          ...
          return (
            <li key={st.label} onMouseEnter={() => setHi(i)} onMouseLeave={() => setHi(null)} className={cn("transition-opacity", hi !== null && hi !== i && "opacity-45")}>
              <div className="flex items-center justify-between text-[12px] font-bold">
                <span>{st.label}</span>
                <span className="font-mono tabular-nums text-muted-foreground">{st.value.toLocaleString()}<span className="ml-1.5">({(share * 100).toFixed(0)}%)</span></span>
              </div>
              <div className="relative mt-1 h-8 rounded-md bg-muted/60">
                {i > 0 && drop > 0 && (
                  <div aria-hidden className="absolute inset-y-0 right-0 flex items-center justify-end pr-2" style={{ width: `${((prev - st.value) / top) * 100}%` }}>
                    <span className="font-mono text-[9px] font-black uppercase tracking-[0.1em] text-[hsl(var(--err))]">−{drop.toLocaleString()}</span>
                  </div>
                )}
