// Vendored from Bklit UI (MIT) — https://github.com/bklit/bklit-ui · packages/ui/src/charts/scatter-chart-shell.tsx — fetched 2026-09-01
// Adapted: import rewrites (@/lib/utils → relative); added optional xLabelFmt for numeric day-of-stock x labels; chart tokens bridge to APP SYSTEM vars
"use client";
...
export interface ScatterChartInnerProps {
  ...
  lines: LineConfig[];
  /** Optional x-label formatter — numeric x encodings map to domain-true labels. */
  xLabelFmt?: (d: Record<string, unknown>) => string;
...
export function ScatterChartInner({
  ...
  const dateLabels = useMemo(
    () =>
      data.map((d) => (xLabelFmt ? xLabelFmt(d) : shortDateFmt.format(xAccessor(d)))),
    [data, xAccessor, xLabelFmt]
  );
