    ...
    <AnimatePresence>
      {!streaming && (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className={cn("flex items-center gap-1 pt-2 font-sans", className)}>
          <Button type="button" variant="ghost" aria-pressed={rating === "up"} aria-label="Good answer" onClick={() => onThumbs?.("up")} className={"group grid size-8 place-items-center rounded-md hover:bg-muted " + (rating === "up" ? "text-[hsl(var(--ok))]" : "text-muted-foreground")}>
            <motion.span animate={rating === "up" ? { scale: [1, 1.3, 1] } : { x: 0 }} whileHover={{ x: -2 }}>{<ThumbsUp className="size-4" />}</motion.span>
          </Button>
          <Button type="button" variant="ghost" aria-pressed={rating === "down"} aria-label="Bad answer" onClick={() => onThumbs?.("down")} className={"grid size-8 place-items-center rounded-md hover:bg-muted " + (rating === "down" ? "text-[hsl(var(--err))]" : "text-muted-foreground")}>
            <motion.span animate={rating === "down" ? { y: [0, 3, 0] } : {}}><ThumbsDown className="size-4" /></motion.span>
          </Button>
          {onEdit && <Button type="button" variant="ghost" aria-label="Edit answer" onClick={onEdit} className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-muted"><Pencil className="size-4" /></Button>}
          {onCopy && (
            <Button type="button" variant="ghost" aria-label="Copy answer" onClick={async () => { await onCopy(); setCopied(true); setTimeout(() => setCopied(false), 1600) }} className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-muted">
          ...
          {onRegenerate && <Button type="button" variant="ghost" aria-label="Regenerate answer" onClick={() => { setSpin((s) => s + 1); onRegenerate() }} className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-muted"><motion.span animate={{ rotate: spin * 360 }} transition={{ duration: 0.7 }}><RefreshCw className="size-4" /></motion.span></Button>}
